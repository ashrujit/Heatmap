...
the Quantower chart as a spatial input surface for a proposed campaign root and
harvest area, then writes a separate JSON draft for `KahnDispatcher` to import.
...
- The JSON output uses Kahn vocabulary (`root_range`, `harvest_range`) instead
  of EAR's `order_context_range` so downstream code cannot silently treat the
  draft as an EAR directive.
