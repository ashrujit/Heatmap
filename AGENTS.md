...
- [`DirectiveSketchProbe/`](DirectiveSketchProbe/AGENTS.md) - Disposable probe indicator that captures an explicitly armed two-box chart sketch and writes a separate JSON draft artifact for possible Dispatcher import research.
- [`SaavikProbe/`](SaavikProbe/AGENTS.md) - Passive Kahn chart-sketch input. Captures root/probe and harvest/TP rectangles into a separate draft JSON for KahnDispatcher; never dispatches or writes active runtime controls.
- [`KahnDispatcher/`](KahnDispatcher/AGENTS.md) - Minimal WinForms operator console for Kahn. Imports SaavikProbe drafts, derives the arena from root plus harvest, validates/dispatches through `kahnctl.py`, and exposes Kahn CANCEL/FLAT controls.
- [`GexBotMcp/`](GexBotMcp/AGENTS.md) - Exploratory MCP adapter for GexBot options-derived context. Supplies normalized futures-space GEX levels and context markers for Prep/Saavik/Kahn waypoint research, not execution permission.
