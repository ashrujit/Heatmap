...
This repo hosts a suite of Quantower indicators built as standalone .NET 10 DLLs. Each indicator lives in its own subfolder, has its own `.csproj`, and deploys to `<QT_ROOT>\Settings\Scripts\Indicators\<Name>\` via the project file's `OutputPath`.
...
1. **No blocking on UI thread.** Callbacks fire on broker thread. For events that mutate shared state, enqueue and drain on UI thread inside `OnUpdate`. `OnPaintChart` is also UI-thread.
2. **Read the canonical book from `Symbol.DepthOfMarket`, do not maintain a delta-merged BookState.** As of the Option-A refactor (2026-05-09), all three suite indicators sample `Symbol.DepthOfMarket.GetDepthOfMarketAggregatedCollections(...)` on each sample tick. QT eats every L2 delta and every `DOMQuote` full-snapshot itself; the canonical book is what its DOM panel uses. Maintaining our own delta-merged book made orphan-level corruption (the 2026-05-08 ref_tick bug — see RESEARCH_LOG) possible whenever a `Closed` event was missed. Reading from QT's book makes that class of bug structurally impossible. Subscribe to `Symbol.NewLevel2` only as a freshness heartbeat (timestamp last event for the IsFresh / STALE-badge logic) — don't process the events.
3. **Tick-keyed dictionaries, not double-keyed.** Float-equality across independently computed prices breaks `Dictionary` lookup. Convert `Level2Item.Price` to `long` ticks via `(long)Math.Round(price / tickSize)` whenever you key by price.
4. **Quantower synthesizes pseudo-L2 events** from L1 best-bid/ask changes (`id="generated_from_level1"`, NaN price/size). QT's DepthOfMarket already handles these internally; the heartbeat handler doesn't need to filter, but any code that consumes raw `Level2Quote` does.
5. **Forward-only.** No historical L2. Indicators that need history accumulate it from live data and warm up over time.
6. **Unsafe pixel writes are scoped narrowly.** `LockBits` + raw `int*` writes are ~10× faster than per-cell `FillRectangle`, but only worth it for the heaviest paint paths. Don't extend the unsafe scope beyond the rebuild method that needs it.
7. **Per-frame bitmap caching** for chart-backdrop renders. Rebuild only on data-change, pan/zoom, or rect resize. Cache hit = single `g.DrawImage` blit.
